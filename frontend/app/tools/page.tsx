...
interface MongoToolResponse {
  _id: string
  name: string
  slug: string
  category: 'Finance' | 'Utility' | 'Developer' | 'Text' | 'Image'
  description: string
  tags?: string[] | string
}
...
function ToolsCatalogContent() {
  ...
  const [dbTools, setDbTools] = useState<ToolItem[]>([])
      ...
      try {
        ...
        if (res.ok) {
          ...
          if (Array.isArray(data)) {
            const parsed: ToolItem[] = data.map((t: MongoToolResponse) => {
              const rawCategory = t.category || 'Utility'
              const validCategory: 'Finance' | 'Utility' | 'Developer' | 'Text' = 
                ['Finance', 'Utility', 'Developer', 'Text'].includes(rawCategory)
                  ? (rawCategory as 'Finance' | 'Utility' | 'Developer' | 'Text')
                  : 'Utility'
        ...
        console.error("Failed to fetch database tools:", err)
      } finally {
        setLoading(false)
      }
    }
  ...
  const mergedTools = useMemo(() => {
    ...
    const uniqueMockTools = ALL_TOOLS.filter(t => !dbSlugs.has(t.slug))
    return [...dbTools, ...uniqueMockTools]
  }, [dbTools])
  ...
  }, [mergedTools, activeCategory, searchQuery])
      ...
      <div className="mx-auto w-full max-w-7xl">
        <div className="mb-10 text-left">
          ...
          <p className="mt-2 max-w-2xl text-base text-slate-500 dark:text-slate-400">
            Explore our ecosystem of zero-friction utilities. Safe client-side execution with zero subscription costs.
          </p>
          {searchQuery && (
        ...
        {loading ? (
          <div className="py-20 text-center text-xs text-slate-400">
            ...
            Compiling active tools registry...
          </div>
            ...
            {filteredTools.map((tool) => {
              ...
              return (
                ...
                >
                  <div>
                    <h3
                      className={`mb-1.5 text-lg font-semibold text-gray-700 dark:text-gray-200 transition-colors ${
                        isGreenHighlight
                          ? 'dark:text-[#A6FF5D]'
