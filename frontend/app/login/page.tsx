...
export default function LoginPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const endpoint =
        activeTab === "signin" ? "/api/auth/login" : "/api/auth/signup";
      ...
      if (response.ok && data.success) {
        if (activeTab === "signin") {
          ...
          const loginData = (await loginResponse.json()) as AuthenticationResponse;

          if (loginResponse.ok && loginData.success) {
            localStorage.setItem("active_user", JSON.stringify({ email }));
            router.push("/tools");
          } else {
            // Fallback: account was created but auto-login failed for some reason
            setActiveTab("signin");
