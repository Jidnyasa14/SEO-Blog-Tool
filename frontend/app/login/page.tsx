...
export default function LoginPage() {
  ...
  body: JSON.stringify({ email, password }),
      ...
      if (response.ok && data.success) {
        if (activeTab === "signin") {
          // Store user details locally so components can dynamically extract initials for the avatar
          localStorage.setItem("active_user", JSON.stringify({ email }));
          
          // Clear route compilation states and force hard redirect explicitly to /tools
          router.push("/tools");
        } else {
          setActiveTab("signin");
        ...
        if (data.errors && Array.isArray(data.errors)) {
          setError(data.errors[0]?.message || "Input verification failed.");
        } else {
          setError(data.message || "Authentication process rejected.");
        }
      }
    } catch (err) {
...
//     if (res.data.success) {
//       // Save the user data locally so components can read who logged in
//       localStorage.setItem('active_user', JSON.stringify(res.data.user));
      
//       // Redirect your administrator to the dashboard panel
//       router.push('/admin');
//     }
//   } catch (err) {
