...
const CATEGORY_REGISTRY: Record<string, CategoryData> = {
  finance: {
    ...
    blogs: [
      { title: 'Understanding EMI: How Loan Amortization Works', slug: 'understanding-emi-debt-management', excerpt: 'Demystify your monthly loan payments. Learn how banks calculate your principal and interest distribution over time.', readTime: '5 min' },
      { title: 'The Power of Compounding: Why You Should Start a SIP Today', slug: 'how-sip-accumulates-wealth', excerpt: 'Discover how small, systematic monthly investments grow into substantial wealth over long horizons.', readTime: '6 min' },
      { title: 'A Complete Beginner Guide to GST Structure and Slabs', slug: 'mathematics-compound-interest-yields', excerpt: 'Break down the complexities of goods and services taxes, input tax credits, and standard tariff brackets.', readTime: '4 min' }
    ]
  },
  developer: {
    name: 'Developer Utilities',
    ...
    blogs: [
      { title: 'Demystifying JSON Web Tokens (JWT) Architecture', slug: 'developer-guide-jwt-architecture', excerpt: 'Learn how modern authentication systems utilize stateless signed tokens to transmit user data securely.', readTime: '7 min' },
      { title: 'The Ultimate Guide to Regular Expression Patterns', slug: 'demystifying-event-loops-regex-parsers', excerpt: 'Stop guessing string matching layouts. Master syntax tokens, anchors, lookaheads, and capture flags.', readTime: '8 min' },
      { title: 'Understanding UUID Version 4 and Cryptographic Randomness', slug: 'mastering-cron-expressions-automation', excerpt: 'Why are collision chances practically zero? Peek inside version 4 unique identifier creation rules.', readTime: '4 min' }
    ]
  },
  utility: {
    name: 'General Utility Engines',
    ...
    blogs: [
      { title: 'The Anatomy of a QR Code: How Do Matrix Codes Work?', slug: 'secure-password-strings-importance', excerpt: 'From quiet zones to position indicators, explore the binary data matrix handling our mobile actions.', readTime: '5 min' },
      { title: 'Password Security 101: Creating Strong, Unbreakable Passphrases', slug: 'data-serialization-workflows-conversion', excerpt: 'Protect your accounts from brute force dictionary matrix scans using modern entropy practices.', readTime: '6 min' }
    ]
  },
  text: {
    name: 'Text Analytics Tools',
    ...
    blogs: [
      { title: 'SEO Copywriting: How to Optimize Content for Search Engines', slug: 'markdown-guide-technical-documentation', excerpt: 'Learn how to combine target keyword placement with pristine semantic readability to rank higher on search engines.', readTime: '9 min' },
      { title: 'The Importance of Clean URL Slugs in Modern Web SEO', slug: 'linguistic-mechanics-case-conversion-metrics', excerpt: 'Understand why search engine indexes prefer descriptive, keyword-rich, hyphen-separated router addresses.', readTime: '4 min' }
    ]
  }
};
...
export default async function CategoryPage({ params }: PageProps) {
  ...
  const fallback = CATEGORY_REGISTRY[slug] || {
    name: `${slug.charAt(0).toUpperCase() + slug.slice(1)} Ecosystem`,
    desc: `Explore interactive utilities and informational guides categorized under ${slug}.`,
    tools: [],
    blogs: [],
  };

  let combinedTools: ToolItem[] = [...fallback.tools];
  let combinedBlogs: BlogItem[] = [...fallback.blogs];

  try {
    await connectToDatabase();
    const categoryRegex = new RegExp(`^${slug}$`, 'i');

    const dbTools = await Tool.find({ isActive: true, category: categoryRegex }).lean<IMongoTool[]>();
    if (dbTools.length > 0) {
      const dbParsedTools: ToolItem[] = dbTools.map((t) => ({
        name: t.name,
        slug: t.slug,
        desc: t.description || '',
      ...
      const existingSlugs = new Set(dbParsedTools.map((t) => t.slug));
      const mockUnique = fallback.tools.filter((t) => !existingSlugs.has(t.slug));
      combinedTools = [...dbParsedTools, ...mockUnique];
    }

    const dbBlogs = await Blog.find({ status: { $regex: /^published$/i }, category: categoryRegex }).lean<IMongoBlog[]>();
    if (dbBlogs.length > 0) {
      const dbParsedBlogs: BlogItem[] = dbBlogs.map((b) => ({
        title: b.title,
        slug: b.slug,
        excerpt: b.summary || (b.content ? b.content.slice(0, 140) : ''),
        readTime: `${b.readingTime || 5} min`,
      }));

      const existingBlogSlugs = new Set(dbParsedBlogs.map((b) => b.slug));
      const mockBlogUnique = fallback.blogs.filter((b) => !existingBlogSlugs.has(b.slug));
      combinedBlogs = [...dbParsedBlogs, ...mockBlogUnique];
    }
  } catch (err) {
    console.error('Error connecting to DB in category page:', err);
  }
      ...
      <main className="w-full flex-grow text-slate-900 dark:text-white">
        <div className="border-b border-slate-200 dark:border-white/10 bg-white/40 dark:bg-zinc-950/20">
          <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8 text-left">
            <nav className="mb-3 flex items-center space-x-2 text-xs font-semibold uppercase tracking-wider text-slate-400">
              <Link href="/tools" className="transition hover:text-violet-600 dark:hover:text-[#A6FF5D]">Categories</Link>
              <FaChevronRight className="text-[9px]" />
              <span className="text-violet-600 dark:text-[#A6FF5D] font-bold">{fallback.name}</span>
            </nav>
            <h1 className="font-['Sora'] text-3xl font-extrabold tracking-tight sm:text-4xl">
              {fallback.name}
            </h1>
        ...
        <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6 lg:px-8 space-y-16 text-left">
          {/* SECTION 1: INTERACTIVE TOOLS */}
          <section className="space-y-6">
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/10 pb-3">
              <FaWrench className="text-violet-600 dark:text-[#A6FF5D] text-sm" />
              <h2 className="text-xl font-bold tracking-tight">Available Interactive Tools ({combinedTools.length})</h2>
            </div>
            ...
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
              {combinedTools.map((tool) => (
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 border-t border-slate-100 pt-3 dark:border-white/10">
                    {tool.tags.slice(0, 3).map((tag) => {
                      const styleClass = TAG_COLORS[tag.toLowerCase()] || TAG_COLORS.default;
                      return (
                        <span
                          key={tag}
                          className={`rounded-md px-2 py-0.5 text-[11px] font-medium uppercase tracking-wider ${styleClass}`}
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-2 border-b border-slate-200 dark:border-white/10 pb-3">
              <FaBookOpen className="text-emerald-500 text-sm" />
              <h2 className="text-xl font-bold tracking-tight">Informational Guides ({combinedBlogs.length})</h2>
            </div>
