...
export default function BlogListingPage() {
        ...
        if (response.ok) {
          ...
          if (Array.isArray(data)) {
            const mappedData: BlogArticle[] = data
              .filter((item: MongoBlogResponse) => item.status === 'published' || item.status === 'Published')
              .map((item: MongoBlogResponse) => ({
                ...
                tags: typeof item.tags === 'string' 
                  ...
                  : Array.isArray(item.tags) 
                    ? item.tags 
                    : [],
                imageUrl: item.image || item.imageUrl || "https://images.unsplash.com/photo-1454165804606-c3d57bc86b40?w=600&auto=format&fit=crop&q=80",
                author: { name: "Toolverse", bio: "IT Services and Consulting company" }
              }));
  ...
  const mergedArticles = useMemo(() => {
    ...
    const uniqueMocks = MOCK_ARTICLES.filter(a => !dbSlugs.has(a.slug));
    return [...dbArticles, ...uniqueMocks];
  }, [dbArticles]);
  ...
  }, [mergedArticles, activeCategory, searchQuery]);
